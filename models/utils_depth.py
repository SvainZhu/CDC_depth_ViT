...
class Dataset_Csv(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        face_temp = self.crop_face_from_scene(img, bbox_path, face_scale)
        face = self.transform(face_temp)
        map_temp = cv2.imread(map_path, 0)
        map = cv2.resize(self.crop_face_from_scene(map_temp, bbox_path, face_scale), (32, 32))
        label = torch.LongTensor([self.labels[index]])  # (labels) LongTensor are for int64 instead of FloatTensor

        # print(X.shape)
        return img, map, label
